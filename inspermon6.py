	...
	while inspermon_vida and inspermon_selvagem_vida > 0:
		...
		if atacar_fugir == "atacar":
			...
			if inspermon_selvagem_vida < 0:
				venceu = "Você venceu a batalha e o inspèrmon oponente desmaiou"
				capturar = input("Digite 1 para capturar o inspèrmon {0} ou 2 para prosseguir sem capturá-lo: ".format(inspermon_selvagem["nome"]))
				if capturar == "1":
					capturado_nao = random.choice(lista_capturado_nao)
					if capturado_nao in lista_capturado_nao[0]:
						print("Você capturou o inspèrmon {0} com sucesso!")
						time.sleep(1)
						insperdex.append(inspermon_selvagem)
						inspermon["xp"] = inspermon["xp"] + 5
						print("O inspermon {0} ganhou 5 XP!!!".format(inspermon["nome"]))
						return venceu
					if capturado_nao in lista_capturado_nao[1]:
						print("{0}, você não conseguiu capturar o inspèrmon {1} e ele se regenerou\n".format(nome,inspermon_selvagem["nome"]))
						time.sleep(3)
						print("ATAQUE SURPRESA!\n")
						time.sleep(1)
						print("Ataque do adversário multiplicado por 2x\n")
						time.sleep(1)
						ataque_nao = inspermon_selvagem["ataque"]*2 - inspermon["defesa"]
						print(random.choice(lista_onoma))
						inspermon_vida = inspermon_vida - ataque_nao
						print("{0} tirou {1} de vida de seu inspèrmon {2} e seu novo valor de vida ficou {3}\n".format(inspermon_selvagem["nome"],ataque_nao,inspermon["nome"],inspermon_vida))
						time.sleep(4)
						if inspermon_vida < 0:
							gameover = "GAME OVER"
							return gameover
...
lista_capturado_nao = ["Você conseguiu capturar o inspèrmon com sucesso!","O inspèrmon voltou de seu desmaio e prepara um ataque contra você"]

#Inspèrmons iniciais

inspermon_quatamander = {"nome":"Quatamander", "ataque":90, "vida": 180,"defesa": 18,"xp":0}
...
print("Estes são os Inspèrmons iniciais disponíveis: \n")
...
while True:
	...
	if passear_dormir == "p":
		...
		if batalha_passeio in lista_encontrou[0]:
			...
			if luta == "Você venceu a batalha e o inspèrmon oponente desmaiou":
				print("Você venceu a batalha e o inspèrmon oponente desmaiou")
